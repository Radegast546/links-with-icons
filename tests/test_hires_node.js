...
function testHiRes(ext, imgListIdx) {
    const psScript = `
...
"@ -ReferencedAssemblies System.Drawing
        Write-Output ([ShellIcon]::GetBase64Icon('${ext}', $true, ${imgListIdx}))
    `;
    ...
    exec(`powershell -ExecutionPolicy Bypass -EncodedCommand ${encodedCommand}`, (error, stdout, stderr) => {
        ...
        console.log(`ext=${ext} imgList=${imgListIdx}: error=${!!error} outLen=${out.length} stderr=${stderr.substring(0,100)}`);
        if (error) console.log("  Error:", error.message.substring(0, 200));
    });
}

testHiRes('.xlsx', 0);  // small 16
testHiRes('.xlsx', 1);  // large 32
testHiRes('.xlsx', 2);  // extra 48
testHiRes('.xlsx', 4);  // jumbo 256
